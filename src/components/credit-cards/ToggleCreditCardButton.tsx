...
import { toggleCreditCardStatusAction } from "@/actions/credit-cards-actions";

type ToggleCreditCardButtonProps = {
  creditCardId: string;
  creditCardName: string;
  active: boolean;
};

export function ToggleCreditCardButton({
  creditCardId,
  creditCardName,
  active,
}: ToggleCreditCardButtonProps) {
  const [isOpen, setIsOpen] = useState(false);

  const actionLabel = active ? "Inativar" : "Ativar";

  const description = active
    ? "Esse cartão não deverá ser usado em novos lançamentos, mas continuará no histórico."
    : "Esse cartão voltará a aparecer como opção de uso no sistema.";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-[#070A12] p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm leading-6 app-muted-text">
              Você está prestes a {actionLabel.toLowerCase()} o cartão{" "}
              <strong className="text-white">{creditCardName}</strong>.{" "}
              {description}
            </p>

            <form
              action={toggleCreditCardStatusAction}
