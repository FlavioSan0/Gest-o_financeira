...
import { Edit3, EyeOff, Loader2, Power, Trash2, X } from "lucide-react";
import {
  deleteAccountAction,
...
type AccountActionsButtonProps = {
  account: {
    id: string;
    name: string;
    type: AccountType;
...
export function AccountActionsButton({ account }: AccountActionsButtonProps) {
  ...
  function handleToggle() {
    ...
    startTransition(async () => {
      try {
        const result = await toggleAccountStatusAction(account.id);

        if (!result?.success) {
          setErrorMessage("Nao foi possivel atualizar essa conta.");
          return;
        }
  ...
  function handleEdit(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        ...
        setErrorMessage("Erro ao salvar a conta. Confira os dados.");
      }
    });
  }
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        ...
        setErrorMessage("Erro ao excluir a conta. Tente novamente.");
      }
    });
  }
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-xl font-black tracking-[-0.03em] text-white">
                  {mode === "edit" ? "Editar conta" : "Excluir conta?"}
                </h3>
                <p className="mt-1 text-sm app-muted-text">
                  {mode === "edit"
                    ? "Atualize nome, tipo, saldo inicial e status."
                    : "A exclusao so acontece quando nao ha lancamentos vinculados."}
                </p>
              </div>
            ...
            {mode === "edit" ? (
              <form action={handleEdit} className="mt-5 grid gap-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      name="initialBalance"
                      defaultValue={formatInputCurrency(
                        account.rawInitialBalance,
                      )}
                      inputMode="decimal"
                      className="finance-input"
                    />
              ...
              <div className="mt-5 grid gap-4">
                <p className="text-sm leading-6 app-muted-text">
                  Se a conta <strong className="text-white">{account.name}</strong>{" "}
                  tiver lancamentos, ela sera bloqueada para exclusao. Nesse
